...
@Component({
  selector: 'app-teams-link-account',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
      ...
      <mat-card class="link-card">
        @if (loading) {
          ...
          <div class="success-state">
            ...
            <p>Your Microsoft Teams account has been successfully linked to your Decision Records account.</p>
            <p class="hint">You can now use &#64;DecisionRecords commands in Teams to create and view decisions.</p>
            <div class="next-steps">
              <h4>Try these commands in Teams:</h4>
              <ul>
                <li><code>&#64;DecisionRecords</code> - Interactive menu with buttons</li>
                <li><code>&#64;DecisionRecords create</code> - Create a new decision</li>
                <li><code>&#64;DecisionRecords list</code> - List recent decisions</li>
                <li><code>&#64;DecisionRecords search [query]</code> - Search decisions</li>
              </ul>
            </div>
          ...
          <mat-card-content>
            <div class="workspace-info">
              ...
              <div>
                ...
                <p class="workspace-detail">Microsoft 365 Tenant</p>
              </div>
            </div>
            ...
            @if (tokenData.is_logged_in) {
              ...
              <div class="logged-in-flow">
                ...
                <p class="instruction">
                  Click below to link your Teams account to your Decision Records account.
                  This will allow you to use &#64;DecisionRecords commands in Teams.
                </p>

                @if (tokenData.teams_email && tokenData.teams_email !== tokenData.user_email) {
                  <div class="email-mismatch-notice">
                    <mat-icon>info</mat-icon>
                    <p>
                      Your Teams email ({{ tokenData.teams_email }}) is different from your
              ...
              <div class="login-flow">
                <p class="instruction">
                  To link your Teams account, please sign in to Decision Records first.
                </p>

                @if (tokenData.teams_email) {
                  <div class="email-hint">
                    <mat-icon>mail</mat-icon>
                    <p>Your Teams email: <strong>{{ tokenData.teams_email }}</strong></p>
                  </div>
                }

                <!-- Social Sign-in Options -->
                @if (teamsOidcEnabled) {
                  <button mat-flat-button class="teams-signin-btn" (click)="signInWithMicrosoft()">
                    <img src="/assets/teams-logo.svg" alt="Microsoft" class="teams-logo">
                ...
                @if (teamsOidcEnabled || googleOauthEnabled) {
  ...
  styles: [`
    ...
    .teams-signin-btn:hover {
      background: #6264A7 !important;
      box-shadow: 0 4px 12px rgba(85, 88, 175, 0.3);
    }

    .teams-signin-btn .teams-logo {
      width: 22px;
      height: 22px;
    }
...
export class TeamsLinkAccountComponent implements OnInit {
  loading = true;
  ...
  ngOnInit(): void {
    ...
    if (!this.token) {
      this.error = 'No link token provided. Please click the "Link Account" button in Teams again.';
  ...
  private checkTeamsOidcStatus(): void {
    this.http.get<{ enabled: boolean }>('/api/auth/teams-oidc-status').subscribe({
      next: (response) => {
        this.teamsOidcEnabled = response.enabled;
      },
      error: () => {
        this.teamsOidcEnabled = false;
      }
    });
  }
  ...
  signInWithMicrosoft(): void {
    ...
    if (this.token) {
      sessionStorage.setItem('teams_link_token', this.token);
    }
    // Redirect to Teams OIDC with return URL to this page
    const returnUrl = `/teams/link?token=${this.token}`;
    window.location.href = `/auth/teams/oidc?return_url=${encodeURIComponent(returnUrl)}`;
  }
  ...
  validateToken(): void {
    this.http.post<LinkTokenData>('/api/teams/link/validate', { token: this.token }).subscribe({
      next: (data) => {
        if (data.valid) {
          this.tokenData = data;
        } else {
          this.error = data.error || 'Invalid or expired link token. Please click the "Link Account" button in Teams again.';
  ...
  completeLink(): void {
    ...
    this.http.post<any>('/api/teams/link/complete', { token: this.token }).subscribe({
      next: (response) => {
  ...
  goToLogin(): void {
    ...
    this.router.navigate(['/'], { queryParams: { teams_link: 'true' } });
  }
  ...
  goToSignup(): void {
    ...
    this.router.navigate(['/'], { queryParams: { teams_link: 'true', signup: 'true' } });
  }
  ...
  closeWindow(): void {
    // Try to close the window (works if opened by Teams)
    window.close();
    // If window didn't close, show a message
    setTimeout(() => {
      this.snackBar.open('You can close this tab and return to Teams', 'OK', { duration: 5000 });
    }, 500);
  }
}
